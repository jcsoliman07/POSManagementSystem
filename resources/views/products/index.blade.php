...
<x-layout>
                ...
                @foreach ($products as $product)
                    <div data-category="{{ $product->category->name }}" class="product-card group bg-white rounded-xl shadow-lg overflow-hidden hover:scale-105 hover:translate-y-1 transition-transform duration-300 mb-8 ">
                        <div class="relative h-48">
                            ...
                            <span class="absolute top-3 right-3 bg-custom-yellow text-custom-gray px-2 py-1 rounded-full font-bold text-sm">Best Seller</span>
                        </div>
                        <div class="p-4">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                {{-- <button class="jollibee-red text-white px-3 py-1 rounded-lg text-sm">Manage</button> --}}
    ...
    {{-- <script>
        ...
        document.querySelectorAll('.product-card').forEach(card => {
            const manageBtn = card.querySelector('button');
            manageBtn.addEventListener('click', function(e) {
                e.stopPropagation();
                const productName = card.querySelector('h3').textContent;
                alert(`Managing product: ${productName}`);
                // In a real app, this would open a product management modal
            });
        }); --}}
